      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 top-14 md:top-16 bg-white z-40 flex flex-col">
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-0.5">
            {(() => {
              const activeSeller = currentSeller || (isSeller && user ? user : null);
              if (!activeSeller) return null;
              const sellerHeroes = heroes
                .filter((h) => h.seller_id === activeSeller.id && h.is_active)
                .sort((a, b) => a.sort_order - b.sort_order);
              return (
                <>
                  <Link
                    to={`/brand/${encodeURIComponent(activeSeller.full_name.toLowerCase())}`}
                    className="block px-3 py-1.5 text-sm font-medium rounded-[4px] text-black"
                  >
                    {activeSeller.full_name}
                  </Link>
                  {sellerHeroes.length > 0 && (
                    <div className="pl-4 pb-2 space-y-0.5">
                      {sellerHeroes.map((hero) => {
                        const isHeroOpen = openHeroId === hero.id;
                        const items = Array.isArray(hero.subtitle) ? hero.subtitle : [];
                        return (
                          <div key={hero.id}>
                            <button
                              onClick={() => setOpenHeroId(isHeroOpen ? null : hero.id)}
